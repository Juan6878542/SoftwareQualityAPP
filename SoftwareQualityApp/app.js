...
return {inputs:v,breakdown:{calidad:{score:clampScore(avgCalidad),weight:wCalidad},codigo:{score:clampScore(avgCodigo),weight:wCodigo},programacion:{score:clampScore(avgProgramacion),weight:wProgramacion},final:{score:finalScore}}};
}
function renderResult(res){
const area=document.getElementById('resultArea');
const final=res.breakdown.final.score;
area.innerHTML=`<h3>Resultado</h3><p>Puntuación final (0 - 5): <strong>${final}</strong></p>
...
<details><summary>Ver datos completos (JSON)</summary><pre>${JSON.stringify(res,null,2)}</pre></details>`;}
document.getElementById('calculateBtn')?.addEventListener('click',()=>{renderResult(calculate());});
